...
<body class="bodi">
                    ...
                    <a href="{{ route('profile.edit') }}" class="profile-link d-flex align-items-center">
                        <div>
                            @if(Auth::user() && Auth::user()->foto && file_exists(public_path('storage/profile_images/' . Auth::user()->foto)))
                                <img src="{{ asset('storage/profile_images/' . Auth::user()->foto) }}" alt="Foto Profil" class="rounded-circle" style="width: 32px; height: 32px; object-fit: cover;" />
                            @else
                        ...
                        <span class="username fw-semibold ms-2">{{ Auth::user()->nama ?? '' }}</span>
    ...
    <script>
        // Sidebar gesture & dragbar logic (sama seperti admin)
        let touchStartX = 0;
        let touchEndX = 0;
        ...
        document.querySelector('.overlay').addEventListener('click', function() {
            ...
            document.querySelector('.overlay').classList.remove('active');
            document.querySelector('.main-container').classList.remove('dimmed');
            document.getElementById('sidebarDragbar').style.display = 'flex';
        });
    </script>
</body>
</html>
